    ...
    public:
        ...
        int parse_header(std::size_t last_len) {
            ...
            if (pos != std::string_view::npos) {
                queries_ = parse_query(
                        std::string_view{raw_url_}.substr(pos + 1, url_len_ - pos - 1));
                url_len_ = pos;
            }
        ...
        std::string_view raw_url() { return raw_url_; }
        ...
        size_t header_len() const { return header_len_; }
        ...
        bool at_capacity() { return (header_len_ + body_len_) > MaxSize; }

        bool at_capacity(size_t size) { return size > MaxSize; }
        ...
        void set_left_body_size(size_t size) { left_body_len_ = size; }
        ...
        const char *current_part() const { return &buf_[header_len_]; }

        const char *buffer(size_t size) const { return &buf_[size]; }
        ...
        bool has_body() const { return body_len_ != 0 || is_chunked_; }

        bool is_http11() { return minor_version_ == 1; }

        int minor_version() { return minor_version_; }
        ...
        bool body_finished() { return left_body_len_ == 0; }

        bool is_chunked() const { return is_chunked_; }

        bool has_gzip() const { return has_gzip_; }

        void reduce_left_body_size(size_t size) { left_body_len_ -= size; }
        ...
        std::string_view get_header_value(std::string_view key) const {
            ...
            auto it = std::find_if(
                    copy_headers_.begin(), copy_headers_.end(), [key](auto &pair) {
        ...
        bool is_multipart_file() const {
            ...
            bool has_content_type =
                    (multipart_headers_.find("Content-Type") != multipart_headers_.end());
        ...
        void set_multipart_headers(const multipart_headers &headers) {
            for (auto pair: headers) {
                multipart_headers_[std::string(pair.first.data(), pair.first.size())] =
        ...
        int parse_chunked(size_t bytes_transferred) {
            auto str =
                    std::string_view(&buf_[header_len_], bytes_transferred - header_len_);
        ...
        void set_state(data_proc_state state) { state_ = state; }
        ...
        void set_http_type(content_type type) { http_type_ = type; }

        content_type get_content_type() const { return http_type_; }
...
#ifdef CINATRA_ENABLE_GZIP
            gzip_str_.clear();
            r = gzip_codec::uncompress(std::string_view(&buf_[header_len_], body_len_),
                                       gzip_str_);
#endif
            return r;
        }
        ...
        std::weak_ptr<session> get_session(const std::string &name) {
            ...
            if (iter != cookies.end()) {
                ref = session_manager::get().get_session(
        ...
        std::weak_ptr<session> get_session() { return get_session(CSESSIONID); }

        void set_range_flag(bool flag) { is_range_resource_ = flag; }

        bool is_range() const { return is_range_resource_; }
        ...
        void set_range_start_pos(std::string_view range_header) {
            if (is_range_resource_) {
                ...
                auto pos_str =
                        range_header.substr(l_str_pos + 1, r_str_pos - l_str_pos - 1);
