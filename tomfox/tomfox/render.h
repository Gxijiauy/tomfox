...
namespace render {
    ...
    class object {
        ...
        struct holder_impl : holder {
            ...
            virtual bool cond() const override { return cond_<T>(0); }

            template<class U,
                    int = sizeof(std::declval<U>().begin(), std::declval<U>().end())>
            ...
            template<class U, int = sizeof(std::declval<std::stringstream &>()
                    << std::declval<U>())>
            ...
            virtual std::string str() const override { return str_<T>(0); }

            template<class U,
                    int = sizeof(std::declval<U>()[std::declval<std::string>()])>
    ...
    public:
        parse_error(std::string message, int line_number, std::string line1,
                    std::string line2)
                : message_(std::move(message)), line_number_(line_number), line1_(line1),
                  line2_(line2) {
            {
                std::stringstream ss;
    ...
    namespace internal {
        ...
        public:
            ...
            parser(Iterator first, Iterator last, const std::string &str)
                    : next_(first), last_(last), line_number_(1), src_str_(str) {
                assert(first != last);
                current_ = next_++;
            }
            ...
            explicit operator bool() const { return current_ != last_; }
            ...
            bool has_next() const { return next_ != last_; }
            ...
            std::string read_inline_str() { return read_include_str(); }

            void eat(char c) {
                if (peek() != c)
                    throw std::string("Unexpected character ") + peek() +
        ...
        static object get_variable(parser<Iterator> &p, const Dictionary &dic,
                                   tmpl_context &ctx, bool skip) {
            p.skip_whitespace();
            if (skip) {
                p.read_variable();
                while (p.peek() == '.') {
        ...
        static object get_variable0(const Dictionary &dic, tmpl_context &ctx, bool skip,
                                    std::string_view str, const std::string &val) {
            if (skip) {
                return object();
            }
        ...
        struct output_type {
            ...
            void flush() { ios << std::flush; }

            void operator<<(std::stringstream &s) { ios << s.rdbuf(); }
        ...
        static void block(parser<Iterator> &p, const Dictionary &dic, tmpl_context &ctx,
                          bool skip, F &out) {
            while (p) {
                ...
                if (c == '}') {
                    ...
                    if (c == '$') {
                        ...
                        // $# comments
                        p.read_while_or_eof([](char peek) { return peek != '\n'; });
                    } else if (c == '{') {
                        p.read();
                        c = p.peek();
                        if (c == '{') {
                        ...
                        if (p.equal(command, "for")) {
                            ...
                            if (not p.equal(in, "in"))
                                throw "Unexpected string \"" + std::string(in.first, in.second) +
                                      "\". It must be \"in\"";
                            object obj = get_variable(p, dic, ctx, skip);
                            p.eat_with_whitespace("{{");
                            ...
                            throw "Unexpected command " +
                                  std::string(command.first, command.second) +
                                  ". It must be \"for\" or \"if\"";
                        }
                    }
                } else {
    ...
    static void parse(Input &&input, Dictionary &&dic) {
        parse(std::forward<Input>(input), std::forward<Dictionary>(dic),
              internal::from_ios(std::cout));
    }

    template<class Dictionary, class Output>
    static void parse(const char *input, Dictionary &&dic, Output &&out) {
        parse(internal::cstring(input), std::forward<Dictionary>(dic),
    ...
    static void to_render_data(const nlohmann::json &json,
                               std::map<std::string, object> &render_map);

    template<typename Object>
    static void to_render_data_impl(const nlohmann::json &json,
                                    Object &&render_data, const std::string &key);
    ...
    static std::string render_string(const std::string &tpl_str,
                                     const nlohmann::json &data) {
        std::stringstream result;
        std::map<std::string, object> render_map;
        to_render_data(data, render_map);
