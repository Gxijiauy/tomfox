...
namespace tomfox {
    ...
    public:
        ...
        response_data post(std::string uri, std::string body) {
            return request(http_method::POST, std::move(uri), req_content_type::json,
                           timeout_seconds_, std::move(body));
        }

        response_data post(std::string uri, std::string body, req_content_type type) {
            return request(http_method::POST, std::move(uri), type, timeout_seconds_,
                           std::move(body));
        }
        ...
        response_data post(std::string uri, std::string body, req_content_type type,
                           size_t seconds) {
            return request(http_method::POST, std::move(uri), type, seconds,
                           std::move(body));
        }

        response_data post(std::string uri, std::string body, size_t seconds,
                           req_content_type type) {
            return request(http_method::POST, std::move(uri), type, seconds,
                           std::move(body));
        }

        response_data request(http_method method, std::string uri,
            ...
            async_request(method, std::move(uri), nullptr, type, seconds,
                          std::move(body));
        ...
        auto async_get(std::string uri, _Callback &&cb) {
            return async_request(http_method::GET, std::move(uri),
                                 std::forward<_Callback>(cb), req_content_type::json,
                                 timeout_seconds_);
        }
        ...
        auto async_get(std::string uri, req_content_type type, _Callback &&cb) {
            return async_request(http_method::GET, std::move(uri),
                                 std::forward<_Callback>(cb), type, timeout_seconds_);
        }
        ...
        auto async_get(std::string uri, _Callback &&cb, req_content_type type,
                       size_t seconds) {
            return async_request(http_method::GET, std::move(uri),
                                 std::forward<_Callback>(cb), type, seconds);
        }
        ...
        template<typename _Callback>
        auto async_get(std::string uri, req_content_type type, size_t seconds,
                       _Callback &&cb) {
            return async_request(http_method::GET, std::move(uri),
                                 std::forward<_Callback>(cb), type, seconds);
        }
        ...
        template<typename _Callback>
        auto async_get(std::string uri, size_t seconds, req_content_type type,
                       _Callback &&cb) {
            return async_request(http_method::GET, std::move(uri),
                                 std::forward<_Callback>(cb), type, seconds);
        }
        ...
        auto async_post(std::string uri, std::string body, _Callback &&cb) {
            return async_request(http_method::POST, std::move(uri),
                                 std::forward<_Callback>(cb), req_content_type::json,
                                 timeout_seconds_, std::move(body));
        }
        ...
        template<typename _Callback>
        auto async_post(std::string uri, std::string body, _Callback &&cb,
                        req_content_type type, size_t seconds) {
            return async_request(http_method::POST, std::move(uri),
                                 std::forward<_Callback>(cb), type, seconds,
                                 std::move(body));
        }
        ...
        template<typename _Callback>
        auto async_post(std::string uri, std::string body, _Callback &&cb,
                        size_t seconds, req_content_type type) {
            return async_request(http_method::POST, std::move(uri),
                                 std::forward<_Callback>(cb), type, seconds,
                                 std::move(body));
        }
        ...
        template<typename _Callback>
        auto async_post(std::string uri, std::string body, req_content_type type,
                        _Callback &&cb) {
            return async_request(http_method::POST, std::move(uri),
                                 std::forward<_Callback>(cb), type, timeout_seconds_,
                                 std::move(body));
        ...
        template<typename _Callback>
        auto async_post(std::string uri, std::string body, size_t seconds,
                        _Callback &&cb) {
            return async_request(http_method::POST, std::move(uri),
                                 std::forward<_Callback>(cb), req_content_type::json,
                                 seconds, std::move(body));
        ...
        template<typename _Callback>
        auto async_post(std::string uri, std::string body, size_t seconds,
                        req_content_type type, _Callback &&cb) {
            return async_request(http_method::POST, std::move(uri),
                                 std::forward<_Callback>(cb), type, seconds,
                                 std::move(body));
        }
        ...
        void async_request_impl(http_method method, std::string uri, callback_t cb,
                                req_content_type type = req_content_type::json,
                                size_t seconds = 15, std::string body = "") {
            bool need_switch = false;
            if (!promise_) { // just for async request, guard continuous async request,
                // it's not allowed; async request must be after last one
                // finished!
                need_switch = sync_;
                sync_ = false;
            }
            ...
            bool need_reset =
                    need_switch || (!init && (uri.find(last_domain_) == std::string::npos));
        ...
        template<typename _Callable_t>
        auto download(std::string src_file, std::string dest_file, _Callable_t &&cb,
                      size_t seconds = 60) {
            return download(std::move(src_file), std::move(dest_file), 0, std::move(cb),
                            seconds);
        }
        ...
        template<typename _Callable_t>
        auto upload(std::string uri, std::string filename, _Callable_t &&cb,
                    size_t seconds = 60) {
            return upload(std::move(uri), std::move(filename), 0,
                          std::forward<_Callable_t>(cb), seconds);
        }
        ...
        auto upload(std::string uri, std::string filename, size_t start,
            ...
            start_ = start;
            return async_request(http_method::POST, uri, std::forward<_Callable_t>(cb),
                                 req_content_type::multipart, seconds, "");
        }
    ...
    private:
        ...
        void send_multipart_msg(const context &ctx) {
            ...
            header_str_.append("Content-Type: multipart/form-data; boundary=")
                    .append(BOUNDARY);
            auto multipart_str =
                    multipart_file_start(fs::path(filename).filename().string());
            auto write_str = build_write_msg(
                    ctx, total_multipart_size(left_file_size, multipart_str.size()));
                ...
                if (!ec) {
                    ...
                    if (is_chunked) {
                        ...
                        if ((size_t) parser_.total_len() <= buf_size) {
                            callback(ec, parser_.status(),
                                     {data_ptr + parser_.header_len(), content_len});
        ...
        std::string multipart_file_start(std::string filename) {
            ...
            multipart_start.append("Content-Disposition: form-data; name=\"" +
                                   std::string("test") + "\"; filename=\"" +
                                   std::move(filename) + "\"" + CRCF);
            multipart_start.append(CRCF);
            return multipart_start;
        }

        size_t total_multipart_size(size_t left_file_size,
                                    size_t multipart_start_size) {
